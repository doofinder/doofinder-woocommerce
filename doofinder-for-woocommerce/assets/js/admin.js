jQuery(() => {
    ...
    const progressBarElement = document.getElementById('doofinder-for-wc-progress-bar');
    const progressBarStatusElement = document.getElementById('doofinder-for-wc-progress-bar-status');
    const additionalMessagesElement = document.getElementById('doofinder-for-wc-additional-messages');
    const indexingError = document.getElementById('doofinder-for-wc-indexing-error');
    if (!button) {
        return;
    }
    ...
    const ajaxIndexContent = () => {
        ...
        console.info('ajaxIndexContent - Current Language: "' + doofinderCurrentLanguage + '"');
        $.ajax({
            type: 'POST',
            dataType: 'json',
    ...
    const ajaxCreateSearchEngines = () => {
        ...
        $.ajax({
            ...
            url: DoofinderForWC.ajaxUrl,
            data: {
                action: 'doofinder_for_wc_process_step_2',
                lang: doofinderCurrentLanguage,
                process_step: '2'
            }
        })
    ...
    const handleError = (response) => {
        ...
        if (errorCount > maxRetries || currentTimeout > maxTimeout || response.data.error) {
            if (response.data.status !== 'invalid_search_engine' &&
    ...
    const setProgressBarStatus = (indexing = false, preparing = false, creatingEngines = false) => {
        console.info('setProgressBarStatus');
        const indexingStatus = progressBarStatusElement.querySelector('.indexing');
        const preparingStatus = progressBarStatusElement.querySelector('.preparing');
        const creatingEnginesStatus = progressBarStatusElement.querySelector('.creating-engines');
        if (indexing) {
    ...
    const initAjaxIndexContent = () => {
        $(button).click(() => {
            ...
            confirmLeavePage();
            if (typeof createSearchEnginesBeforeIndexing !== 'undefined' && createSearchEnginesBeforeIndexing) {
...
jQuery(() => {
    const $ = jQuery.noConflict();
    const cancelButton = document.querySelector('#doofinder-for-wc-cancel-indexing');
    if (!cancelButton) {
        return;
    }
